...
export default function DataPurchasePage() {
  ...
  const { data: products, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get("/catalog/products", { params: { category: "data" } });
      return data.data;
    },
  });
  ...
  async function handleSubmit(e) {
    ...
    try {
      const { data } = await apiClient.post("/orders/data", { productId, recipientNumber });
      setResult(data.data);
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      <div>
        <h1 className="font-display text-2xl font-semibold text-foreground">Buy Data</h1>
        <p className="text-muted-foreground">Affordable data bundles for any network.</p>
      </div>
                  ...
                  {networkProducts.map((p) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{p.metadata?.validity}</p>
                      </div>
